...
- [x] **Step 4 - Verify hosted validation** - after local changes are reviewable,
...
- Step 2: parsed and reviewed the complete workflow. HACS and Hassfest have no
  ...
  suppressed. Hosted execution was deferred to Step 4; final results are below.
...
- The maintainer explicitly selected Apache-2.0 after reviewing the license
  ...
  Step 4 requires both hosted jobs to pass before archival.
...
- The licensed branch was pushed at
  `c0e8fc69cadd91b884294d9562c5e49807116856` and produced
  [Validate run 34590728188](https://github.com/yairabf/home-stock-tracker-ha/actions/runs/34590728188).
  **Hassfest passed; HACS still failed only the license check.** The official
  HACS checker reads `repository_object.attributes["license"]` from GitHub's
  repository metadata. After the feature-branch push, the repository API still
  reports `default_branch: main` and `license: null`; `main` has no license.
  Updated the release instructions to explain this prerequisite.
- The license prerequisite was isolated in commit
  `93ce2a6c00d52ea635168a5ef512a52b282927e8`, which changes only `LICENSE` and `README.md`.
  Its patch applies cleanly to fetched `origin/main` at
  `7a44912aaf44ad4266c8b170dfb4c7172caa4dfd`, confirmed with a temporary index
  and `git apply --cached --check --whitespace=error` without modifying main.
  With the user's separate explicit approval, this two-file prerequisite was
  cherry-picked to `main` as `902c8e1ea3e64279d48df646eb2570c8bb843676` and pushed.
  GitHub's repository API then reported `license.spdx_id: Apache-2.0`.
- Final hosted evidence: rerunning the failed job in
  [Validate run 34590728188](https://github.com/yairabf/home-stock-tracker-ha/actions/runs/34590728188)
  produced **HACS: success**, **Hassfest: success**, and **overall: success**
  for push commit `c0e8fc69cadd91b884294d9562c5e49807116856`.
  HACS logged the feature branch and all eight enabled checks completed,
  including `license`; only `brands` remains ignored pending Feature 2c.
  Hassfest's checkout log records that exact commit and reports
  `Integrations: 1`, `Invalid integrations: 0`.
  Evidence was inspected with `gh run view 34590728188 --repo
  yairabf/home-stock-tracker-ha --json status,conclusion,headSha,jobs,url`
  and the same command with `--log`.
- Completion's final local checks: `.venv/bin/python -m pytest -q tests` passed
  **11 tests** (0.50 seconds); `git diff main --check` passed. Remaining changes
  after hosted verification are release guidance and Blueprint completion
  records. No runtime, metadata, dependency, or validator behavior changed.
